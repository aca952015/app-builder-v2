...
- 当前只允许执行：需求分析、模型收敛、结构化 spec 定义、分析稿落盘、详细 spec 落盘、在最终结构化响应中返回 `planSpec` 和 `interactionContract`。
...
2. `artifacts.generatedSpec`
   写一份中文详细 spec，面向人类审阅。
3. `planSpec`
   在最终结构化响应的 `planSpec` 字段返回一份 JSON 对象；host 会把它写入 `artifacts.planSpec`，作为后续生成阶段唯一事实来源。
4. `interactionContract`
   在最终结构化响应的 `interactionContract` 字段返回一份 JSON 对象；host 会把它写入 `artifacts.interactionContract`，作为后续生成阶段实现关键交互、页面到 API 映射和外部操作的执行契约。

最终结构化响应中的 `planSpec` 是最关键产物。它必须严格符合输入里的 `planSpecSchema`，并且使用结构化定义表达后续生成和验证所需的关键信息。
不要直接用文件写入工具创建或修补 `artifacts.planSpec`；必须把完整对象放在最终结构化响应的 `planSpec` 字段中，由 host 统一落盘。

最终结构化响应中的 `interactionContract` 必须符合输入里的 `hardConstraints.interactionContractValidation.schema`。不要直接用文件写入工具创建或修补 `artifacts.interactionContract`；必须把完整对象放在最终结构化响应的 `interactionContract` 字段中，由 host 统一落盘。
`interactionContract` 顶层必须包含 `flows`、`internalOperations`、`externalOperations` 三个数组；没有对应操作时写空数组。

输入里的 `hardConstraints.planSpecSchemaValidation`、`hardConstraints.interactionContractValidation`、`hardConstraints.environmentVariablePolicyValidation`、`hardConstraints.referenceUsageValidation` 是阻断性硬约束，不是建议项。
...
- 必填字符串字段必须提供非空字符串
- 如果输入的 `externalReferences`、`localReferences` 或 `artifacts.referenceManifest` 中存在已下载本地资料，必须先读取对应 `localPath` 文件，再组装 `artifacts.generatedSpec`、最终结构化响应中的 `planSpec` 和 `interactionContract`
- 外部 API endpoint、认证方式、参数格式/顺序、响应字段、错误码和限制信息必须优先来自已下载本地资料，不能凭模型记忆或远程 URL 猜测

## 结构化 spec 约束

最终结构化响应中的 `planSpec` 至少必须包含并正确填写：
...
- 对当前尚不存在的 `artifacts.analysis`、`artifacts.generatedSpec`，应直接创建，不要因为路径存在于输入里就先尝试读取。
...
- 最终结构化响应中的 `planSpec` 必须先完成到可验证状态，再补充 Markdown 分析稿和详细 spec 的表述一致性。
- 最终结构化响应中的 `interactionContract` 必须同步覆盖 planSpec 中的关键 flows、页面触发 API 和外部操作，且不得依赖手写 JSON 文件落盘。
...
- 最终结构化响应包含完整 `planSpec`，且满足输入中的 schema；host 会将其写入 `artifacts.planSpec`
- 最终结构化响应包含完整 `interactionContract`，且满足输入中的 schema；host 会将其写入 `artifacts.interactionContract`
- 计划产物之间不存在明显冲突
