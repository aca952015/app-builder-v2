...
- 当前允许在严格条件下调用 `task`/子代理，但子代理只是并行提效手段，不是默认委派机制。只有当多个失败项或修补切片彼此独立、可以真正并行推进、文件/职责边界不重叠，且预期节省时间明显大于启动、交接和合并成本时，才允许启用子代理。
- 如果失败根因强耦合、需要顺序诊断、涉及同一共享文件/契约、修补范围很小，或并行不会缩短总修复时间，必须由主代理直接修补，禁止调用子代理。
- 调用子代理时，主代理必须围绕本轮 `validationFailures` 分配精确且不重叠的修补范围；主代理仍负责合并结果、复查失败项、更新 todo，并返回最终结构化响应。
