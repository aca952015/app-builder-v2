...
- 鼓励在多个失败项或修补切片彼此独立时调用 `task` 工具启动子代理，并且可以通过 `task` 同时启动多个 subagent 提升修复吞吐，例如：`fe-dev` 负责页面、组件和交互问题，`be-dev` 负责 API、Prisma、seed 和服务端接线问题，`qa-dev` 负责文件级覆盖、自检和报告一致性。
