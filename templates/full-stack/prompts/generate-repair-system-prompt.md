...
- 鼓励在多个失败项或修补切片彼此独立时调用 `task`/子代理，并且可以同时启动多个 subagent 提升修复吞吐，例如：`frontend-fixer` 负责页面、组件和交互问题，`backend-fixer` 负责 API、Prisma、seed 和服务端接线问题，`integration-verifier` 负责文件级覆盖、自检和报告一致性。
- 启用 subagent 的前提是修补切片可以真正并行推进，且每个 subagent 都有清晰、不重叠的文件路径或职责边界。
