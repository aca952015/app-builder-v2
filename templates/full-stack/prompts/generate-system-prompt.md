...
- 鼓励在有明确并行价值时调用 `task`/子代理，并且可以同时启动多个 subagent 提升生成吞吐，例如：`frontend-implementer` 负责页面、组件、交互和侧边栏，`backend-implementer` 负责 API、Prisma、seed 和服务端接线，`integration-verifier` 负责文件级覆盖、自检和报告一致性。
- 启用 subagent 的前提是至少两个实现或验证切片可以真正并行推进，且每个 subagent 都有清晰、不重叠的文件路径或职责边界。
