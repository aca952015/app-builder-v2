...
- 当前允许在严格条件下调用 `task`/子代理，但子代理只是并行提效手段，不是默认委派机制。只有同时满足以下条件时才允许启用子代理：
  1. 至少两个有价值的实现或验证切片可以真正并行推进，互不等待。
  2. 每个子代理都有明确且不重叠的文件路径或职责边界，低冲突风险。
  3. 预期节省的实现时间明显大于子代理启动、上下文交接和主代理合并成本。
  4. 主代理能低成本检查、合并子代理结果，并继续负责最终自检和结构化返回。
- 如果工作是单文件、强耦合、顺序依赖、低开销、需要持续中间监督，或无法通过并行带来生成提效，必须由主代理直接实现，禁止调用子代理。
- 调用子代理时，主代理必须在任务描述中写清楚唯一事实来源、精确文件/职责范围、禁止越界和最终回报格式；禁止把同一个文件或共享契约拆给多个子代理并行修改。
