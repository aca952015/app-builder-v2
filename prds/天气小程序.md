...
* **设计原则**：采用卡片式 UI 设计，背景随天气情况（晴、阴、雨、雪）自动变换色彩或动态效果，天气图标使用emoji，在浅色、暗色风格中都要清晰可见。
* **适配性**：兼容主流现代浏览器（Chrome, Safari, Edge, Firefox）。

## 6. 技术栈建议
* **API 集成**：使用和风天气 (QWeather) 
* 使用环境中的QWEATHER_API_KEY和QWEATHER_API_HOST，作为API调用的KEY和HOST来源
* API_KEY使用的http header是X-QW-Api-Key
* **API 参考**
