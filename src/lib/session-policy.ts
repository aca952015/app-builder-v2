...
export function buildSessionPolicyDocument(): string {
  return [
    ...
    "- In the initial generate stage, the host may launch default be-dev, fe-dev, and qa-dev subagents before the final merge pass; preserve and merge their work instead of restarting from scratch.",
    "- In generate and generate repair stages, prefer using `task` to launch additional bounded child agents when fe-dev, be-dev, and qa-dev slices can run in parallel with clear non-overlapping ownership.",
