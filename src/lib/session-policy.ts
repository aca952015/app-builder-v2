...
export function buildSessionPolicyDocument(): string {
  return [
    ...
    "## Common Rules",
    "",
    "- The virtual workspace root is `/` and host-managed artifacts stay under `/.workspace/`.",
    ...
    "  - `artifacts.todo` = `/.workspace/todo.md`",
    "  - `artifacts.errorLog` = `/.workspace/error.log`",
    ...
    "- `write_todos` persists the current todo board to `/.workspace/todo.md`; if you need a file fallback, write only `/.workspace/todo.md`, never `/TODO.md` or `/todo.md` at the app root.",
    "- `/.workspace/todo.md` is model-writable progress state. The host monitors it and uses it as the live todo board.",
    "",
    "## Plan Rules",
    "",
    ...
    "- In the initial generate stage, the host may launch default backend, frontend, and integration subagents before the final merge pass; preserve and merge their work instead of restarting from scratch.",
    "- In generate and generate repair stages, prefer using `task` to launch additional bounded child agents when frontend, backend, and verification slices can run in parallel with clear non-overlapping ownership.",
